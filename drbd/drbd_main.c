 ...
 * of it) gets send immediately independently if it is currently
 * corked.
 *
 ...
 * uncorks. That works perfectly to coalesce ack packetes etc..
 * A work item doing two-phase commits needs to override that behavior.
 */
#define SFLAG_FLUSH 0x10000

int __send_command(struct drbd_connection *connection, int vnr,
			  enum drbd_packet cmd, enum drbd_stream drbd_stream_and_flags)
{
	enum drbd_stream drbd_stream = drbd_stream_and_flags & ~SFLAG_FLUSH;
	struct drbd_send_buffer *sbuf = &connection->send_buffer[drbd_stream];
	struct drbd_transport *transport = &connection->transport;
	struct drbd_transport_ops *tr_ops = transport->ops;
	bool corked = test_bit(CORKED + drbd_stream, &connection->flags);
	bool flush = drbd_stream_and_flags & SFLAG_FLUSH;
	int err;
		 ...
		 enum drbd_packet cmd, enum drbd_stream drbd_stream_and_flags)
{
	...
	err = __send_command(connection, vnr, cmd, drbd_stream_and_flags);
	mutex_unlock(&connection->mutex[drbd_stream]);
	return err;
}
